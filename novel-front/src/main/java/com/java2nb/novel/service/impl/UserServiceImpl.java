...
public class UserServiceImpl implements UserService {
    ...
    public UserDetails login(User user) {
        //根据用户名密码查询记录
        SelectStatementProvider selectStatement = select(id, email,username, email, nickName)
                .from(UserDynamicSqlSupport.user)
                .where(username, isEqualTo(user.getUsername()))
    ...
    public User userInfo(Long userId) {
        SelectStatementProvider selectStatement = select(username, email, nickName, isMember, memberValidityTime, userPhoto, userSex, accountBalance)
                .from(user)
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void addAmount(Long userId, byte memberType, int amount) throws ParseException {
        User user = this.userInfo(userId);
        user.setId(userId);
        user.setAccountBalance(user.getAccountBalance() + amount);
        String date;
        if (memberType == 2) {
            date = getDateAfterNMonths(3);
        } else if (memberType == 3) {
            date = getDateAfterNMonths(12);
        } else {
            date = getDateAfterNMonths(1);
        }
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Date MemberValidityTime = format.parse(date);
        user.setIsMember((byte) 1);
        user.setMemberValidityTime(MemberValidityTime);
        userMapper.updateByPrimaryKeySelective(user);
    ...
    public static String getDateAfterNMonths(int n) {
        LocalDate currentDate = LocalDate.now();
        LocalDate dateAfterNMonths = currentDate.plusMonths(n);

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String formattedDate = dateAfterNMonths.format(formatter);
