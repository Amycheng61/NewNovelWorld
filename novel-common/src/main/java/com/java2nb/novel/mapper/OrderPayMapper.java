...
public interface OrderPayMapper {
    ...
    BasicColumn[] selectList = BasicColumn.columnList(id, outTradeNo, tradeNo, payChannel, memberType, totalAmount, userId, payStatus, createTime, updateTime);
    ...
    @Results(id = "OrderPayResult", value = {
            ...
            @Result(column = "member_type", property = "memberType", jdbcType = JdbcType.TINYINT),
    ...
    default int insert(OrderPay record) {
        return MyBatis3Utils.insert(this::insert, record, orderPay, c ->
                c.map(id).toProperty("id")
                        ...
                        .map(memberType).toProperty("memberType")
    ...
    default int insertSelective(OrderPay record) {
        return MyBatis3Utils.insert(this::insert, record, orderPay, c ->
                c.map(id).toPropertyWhenPresent("id", record::getId)
                        ...
                        .map(memberType).toPropertyWhenPresent("memberType", record::getMemberType)
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(OrderPay record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId)
                ...
                .set(memberType).equalTo(record::getMemberType)
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(OrderPay record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId)
                ...
                .set(memberType).equalToWhenPresent(record::getMemberType)
